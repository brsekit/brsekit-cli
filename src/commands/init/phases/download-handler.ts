...
import { mkdtempSync, createWriteStream, existsSync, mkdirSync } from "node:fs";
...
export async function handleDownload(ctx: InitContext): Promise<InitContext> {
  ...
  try {
    ...
    // Write to file
    const fs = await import("node:fs/promises");
    await fs.writeFile(zipPath, Buffer.from(zipball as ArrayBuffer));
    ...
    const extractedFolder = extractedContents.find((name) =>
      name.startsWith(`${GITHUB_ORG}-${STARTER_REPO}`)
    );
