...
import type { InitContext, GitHubRelease } from "../types.js";
...
async function fetchReleases(
  ...
  try {
    ...
    let releases = data as GitHubRelease[];
    ...
    if (error instanceof Error && error.message.includes("Not Found")) {
      throw new Error(
        `Repository ${GITHUB_ORG}/${STARTER_REPO} not found or not accessible.\n` +
...
export async function handleSelection(ctx: InitContext): Promise<InitContext> {
  ...
  try {
    ...
    return { ...ctx, selectedVersion: selected as string };
  } catch (error) {
